  ...
  return (
    <Image
      ...
      unoptimized={
        ...
        !imageUrl.includes("website.anwarululoom.com")
      }
      onError={(e) => {
        if (!errorRef.current) {
...
export default function BlogsPreview({ limit, homePage }: BlogsPreviewProps) {
  ...
  if (isLoadingInitial && filteredBlogs.length === 0) {
    return (
      <section id="blogs" className="bg-background-primary py-16" dir="rtl">
        <div className="max-w-7xl mx-auto px-2 sm:px-4 md:px-6 lg:px-8 space-y-10">
  ...
  return (
    <section
      id="blogs"
      className="bg-background-primary py-12 sm:py-16"
      dir="rtl"
    >
      <div className="max-w-7xl mx-auto px-2 sm:px-4 md:px-6 lg:px-8 space-y-8 sm:space-y-10">
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3">
            {categories.map((category) => (
              <button
                ...
                className={`rounded-full px-3 sm:px-4 md:px-5 py-1.5 sm:py-2 md:py-2.5 text-xs sm:text-sm font-bold transition-all duration-300 border-2 ${
            ...
            {filteredBlogs.map((item, index) => {
              ...
              return (
                <Link
                  ...
                  className="group relative flex h-full flex-col bg-[#e0f2f2] rounded-xl sm:rounded-2xl shadow-md hover:shadow-2xl hover:shadow-emerald-100/50 transition-all duration-300 overflow-hidden border-2 border-[#d0e8e8] hover:border-emerald-400 hover:-translate-y-1"
                  dir="rtl"
                >
                  {/* Top Section - Full Size Image */}
                  <div className="relative h-40 sm:h-44 md:h-48 bg-[#e0f2f2] flex-shrink-0 overflow-hidden">
                    <BlogImage
                      src={item.image}
                    ...
                    <div className="absolute top-2 right-2 sm:top-3 sm:right-3 md:top-4 md:right-4 z-10">
                      <span className="inline-flex items-center gap-1 sm:gap-1.5 px-2 sm:px-2.5 md:px-3 py-1 sm:py-1.5 rounded-lg sm:rounded-xl text-[10px] sm:text-xs font-bold bg-white/95 backdrop-blur-md text-emerald-700 border-2 border-emerald-200/50 shadow-lg">
                        <FileText className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                        <span className="hidden xs:inline">
                          {item.category?.name || t("blog.general")}
                        </span>
                        <span className="xs:hidden">
                          {item.category?.name?.slice(0, 8) ||
                            t("blog.general").slice(0, 8)}
                        </span>
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="relative flex-1 bg-white p-4 sm:p-5 md:p-6 flex flex-col justify-between">
                    ...
                    <div className="relative z-10 space-y-3 sm:space-y-4 flex-1">
                      ...
                      <h2
                        className="text-lg sm:text-xl md:text-2xl font-bold text-emerald-700 group-hover:text-emerald-800 leading-tight line-clamp-2 transition-colors"
                      ...
                      {/* Description */}
                      <p
                        className="text-xs sm:text-sm text-gray-700 leading-relaxed line-clamp-2 sm:line-clamp-3"
                        style={{ fontFamily: "Amiri, serif" }}
                      >
                      ...
                      {/* Date - Small Text */}
                      <div className="flex items-center gap-1.5 sm:gap-2 text-[10px] sm:text-xs text-emerald-600 pt-1 sm:pt-2">
                    ...
                    <div className="relative z-10 flex items-center justify-between">
                      <span
                        className="text-[10px] sm:text-xs text-emerald-700 font-semibold"
                        style={{ fontFamily: "Amiri, serif" }}
                      >
                        {t("blog.readMore")}
                      </span>
                      <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-gradient-to-br from-emerald-50 to-emerald-100 group-hover:from-emerald-100 group-hover:to-emerald-200 flex items-center justify-center transition-all duration-300 group-hover:scale-110 group-hover:shadow-md">
                        <ChevronLeft className="w-3 h-3 sm:w-4 sm:h-4 text-emerald-700" />
                      </div>
                    </div>
                  </div>
                </Link>
